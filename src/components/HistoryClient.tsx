          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '480px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--surface-border)', color: 'var(--text-secondary)' }}>
                <th style={{ padding: '1rem 0.5rem' }}>年度・問題数</th>
                <th style={{ padding: '1rem 0.5rem' }}>正解ステータス</th>
                <th style={{ padding: '1rem 0.5rem' }}>過去の挑戦日時</th>
              </tr>
            </thead>
            <tbody>
              {attempts.length === 0 ? (
                ...
                attempts.map(attempt => {
                  ...
                  return (
                    <tr
                      key={attempt.id}
                      onClick={() => router.push(`/quiz/${attempt.questionId}`)}
                      style={{
